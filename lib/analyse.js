...
function evaluateExp(exp, state) {
  ...
  if (exp.type === 'num') {
    return {type: 'value', valueType: {path: ['u32'], args: []}, value: exp, isLValue: false};
  }
  if (exp.type === 'bool') {
  ...
  if (exp.type === 'sum' || exp.type === 'subt' || exp.type === 'mult' || exp.type === 'div') {
    const left = evaluateExp(exp.left, state);
    if (left.type !== 'value')
      throw new AnalysisError('expected a value', left.loc);
    if (exp.type === 'sum' && !eq(left.valueType.path, ['str']) && !eq(left.valueType.path, ['u32']))
      throw new AnalysisError('expected a string or number', left.loc);
    if (exp.type !== 'sum' && !eq(left.valueType.path, ['u32']))
    ...
    return {
      ...
      value: {type: exp.type, left, right},
      isLValue: false,
    };
  }
  if (exp.type === 'eq') {
  ...
  if (exp.type === 'vec') {
    const items = exp.items.map(value => evaluateExp(value, state));
