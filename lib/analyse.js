...
function analyseImpl(unit) {
  ...
  for (const func of unit.funcs) {
    ...
    for (const st of func.sts) {
      ...
      if (st.type === 'decl') {
        const {decl} = st;
        if (scope.names.has(decl.name.value)) {
          throw new AnalysisError(`name "${decl.name.value}" already exist in scope`, decl.name.loc);
        }
        scope.names.set(decl.name.value, scope.locals.length);
        const init = decl.init && evaluateExp(decl.init, state);
        const type = resolveType(decl.type);
        if (init != null) {
          if (!eq(init.valueType, type))
            throw new AnalysisError('invalid initializer type', decl.init.loc);
        }
        sts.push({
          type: 'decl',
          localIx: scope.locals.length,
          init,
        });
        scope.locals.push({type});
        continue;
