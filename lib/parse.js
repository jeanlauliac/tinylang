  ...
  while (op != null) {
    switch (op) {
      case '.':
        ...
        exp = {type: 'dot_access', target: exp, member};
        break;
      case '(':
        exp = parseFunCall(state, exp);
        break;
      case '[':
        ...
        return {type: 'key_access', target: exp, key};
        break;
    }
...
function parseFunCall(state, callee) {
  const args = [];
  ...
  if (op !== ')') throw new ParseError("expected right paren");
  return {type: 'fun_call', callee, args};
}
