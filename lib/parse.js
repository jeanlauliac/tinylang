...
function parseExp(state) {
  if (parseOp(state, ['&'])) {
    const exp = parseAccessExp(state, true);
...
function parsePrimaryExp(state) {
  ...
  if (exp = parseAccessExp(state, false)) return exp;
  return null;
}
...
function parseAccessExp(state, isRef) {
  const blr = loc(state);
  ...
  while (op != null) {
    switch (op) {
      case '.':
        ...
        break;
      case '(':
        if (isRef) throw new ParseError('unexpected function call in reference');
