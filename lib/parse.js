...
function parseFinalExp(state) {
  ...
  if (exp = parseKeyAccess(state)) return exp;
  return null;
}
...
// TODO: merge with KeyAccess, to support key access on fun call return
// value, ex. `foo()[3]`. (Left-recursion not allowed with this type of parser).
...
function parseKeyAccess(state) {
  ...
  return {type: 'key_access', qualIdent, key};
}
