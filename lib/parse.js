...
function parsePrimarySt(state) {
  ...
  if (decl != null) return {type: 'decl', decl, loc: [blc, loc(state)]};
  const value = parseExp(state);
  if (value != null) return {type: 'exp', value, loc: [blc, loc(state)]};
  return null;
}
...
function parseLocalDecl(state) {
  ...
  if (parseOp(state, /^=$/) === '=') {
    exp = parseExp(state);
    if (exp == null) throw new ParseError('expected initializer expression');
  }
