...
function parse(filePath, code) {
  ...
  try {
    skipSpaces(state);
    let unit = {funcs: [], structs: new Map()};
    let decl = parseDecl(state);
    while (decl != null) {
      if (decl.type === 'func') unit.funcs.push(decl);
      else if (decl.type === 'struct') unit.structs.set(decl.name, decl);
      else throw new ParseError('unexpected declaration');
...
function parseFunction(state) {
  ...
  return {type: 'func', typeName, name, isExported, args, sts};
}
...
const KEYWORDS = new Set(['export', 'if', 'return', 'true', 'false', 'struct']);
...
function parseKeyword(state, set) {
  ...
  if (set.has(keyword.value)) {
    if (!KEYWORDS.has(keyword.value)) throw new Error(`invalid keyword "${keyword.value}"`);
    return keyword;
  }
  restore(state, snap);
  return null;
