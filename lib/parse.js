...
function parseFunArg(state) {
  const type = parseType(state);
  if (type == null) return null;
  const isRef = parseOp(state, ['&']) != null;
  const name = parseIdent(state);
  if (name == null) throw new ParseError('expected argument name');
  return {type, name: name.value, isRef};
}

function parseFunSt(state) {
  if (parseOp(state, [';']) === ';') return {type: 'empty'};
  ...
  const keyword = parseKeyword(state, new Set(['if', 'while']));
  if (keyword == null) return null;
  if (keyword.value === 'if') return parseIfSt(state);
  if (keyword.value === 'while') return parseWhileSt(state);
  return null;
}
...
function parseWhileSt(state) {
  if (parseOp(state, ['(']) !== '(') throw new ParseError("expected left paren");
  const cond = parseExp(state);
  if (cond == null) throw new ParseError('expected condition expression');
  if (parseOp(state, [')']) !== ')') throw new ParseError("expected right paren");
  const body = parseFunSt(state);
  if (body == null) throw new ParseError('expected statement');
...
function parseExp(state) {
  let assignee = parseCompExp(state);
  if (parseOp(state, ['=']) == null) return assignee;
  const from_value = parseExp(state);
  if (from_value == null) throw new ParseError('expected value to be assigned');
  return {type: 'assign', assignee, from_value};
}

function parseCompExp(state) {
  return parseBinOp(state, ['=='], parseSumExp);
}

function parseSumExp(state) {
  return parseBinOp(state, ['+', '-'], parseMultExp);
}

function parseMultExp(state) {
  return parseBinOp(state, ['*', '/'], parsePrimaryExp);
}

function parseBinOp(state, ops, parseSubExp) {
  const blr = loc(state);
  let left = parseSubExp(state);
  if (left == null) return null;
  let op = parseOp(state, ops);
  while (op != null) {
    let right = parseSubExp(state);
    if (right == null) throw new ParseError("expected expression");
    left = {type: 'bin_op', op, left, right, loc: [blr, loc(state)]};
...
function parsePrimaryExp(state) {
  ...
  if (parseOp(state, ['&'])) {
    const exp = parsePrimaryExp(state);
    if (exp == null) throw new ParseError('expected reference expression');
    return {type: 'ref_exp', target: exp};
  };
  let exp;
