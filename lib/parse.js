...
function parseSumExp(state) {
  const blr = loc(state);
  let left = parseMultExp(state);
  if (left == null) return null;
  let op = parseOp(state, ['+', '-']);
  while (op != null) {
    let right = parseMultExp(state);
    if (right == null) throw new ParseError("expected expression");
...
function parseMultExp(state) {
  ...
  if (left == null) return null;
  let op = parseOp(state, MULT_OPS);
  while (op != null) {
    let right = parsePrimaryExp(state);
    if (right == null) throw new ParseError("expected expression");
    left = {type: op === '*' ? 'mult' : 'div', left, right, loc: [blr, loc(state)]};
    op = parseOp(state, MULT_OPS);
  }
  return left;
}
